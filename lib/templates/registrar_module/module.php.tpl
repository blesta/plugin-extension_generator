 ...
 */
class {{class_name}} extends RegistrarModule
{
...
{{function:addService}}{{function:getAdminServiceInfo}}
    ...
    {{function:getCronTasks}}{{function:checkAvailability}}
    ...
    public function getServiceDomain($service)
    {
        if (isset($service->fields)) {
            foreach ($service->fields as $service_field) {
                if ($service_field->key == '{{array:service_fields}}{{if:service_fields.name_key:true}}{{service_fields.name}}{{else:service_fields.name_key}}{{if:service_fields.name_key}}{{array:service_fields}}') {
