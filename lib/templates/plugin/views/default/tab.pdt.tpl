{{array:service_tabs}}
    <div id="{{service_tabs.snake_case_name}}">
        ...
        <?php
        $this->Form->create(
//// {{if:service_tabs.level:staff}}$this->base_uri . 'services/manage/' . $this->Html->ifSet($service_id) . '/{{service_tabs.snake_case_name}}/'{{else:service_tabs.level}}$this->base_uri . 'clients/serviceTab/' . $this->Html->ifSet($client_id) . '/' . $this->Html->ifSet($service_id) . '/{{service_tabs.snake_case_name}}/'{{if:service_tabs.level}}
        );
        ?>
        <div class="col-md-12">
